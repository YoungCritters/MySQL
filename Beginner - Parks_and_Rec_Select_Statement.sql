...
#1. Select everything by using "*":
SELECT * 
FROM parks_and_recreation.employee_demographics;


#2. Selecting a specific column by using specific columnname
SELECT first_name
FROM employee_demographics;

#As you can see from the output, we only have the one column here now and don't see the others

#3. Add some more columns, we just need to separate the columns with commas
...
#4. Names in the select statement will matter, but for this you can put them in any order

SELECT last_name, first_name, gender, age
FROM employee_demographics;

#5. You'll also often see SQL queries formatted like this.
SELECT last_name, 
first_name, 
gender, 
age
...
#6. For example let's take a look at using a calculation in the select statement
...
#7. Math in SQL does follow PEMDAS which stands for Parenthesis, Exponent, Multiplication,
...
#8. One thing I wanted to show you about the select statement in this lesson is the DISTINCT Statement - this will return only unique values in
